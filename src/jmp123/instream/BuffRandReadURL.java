...
public class BuffRandReadURL extends RandomRead {
	...
	public boolean open(String spec, String title) throws IOException {
		...
		if (code < 200 || code >= 300)
			return printErrMsg("URL Connection Fails. ResponseCode: " + code + ", ResponseMessage: "
					+ connection.getResponseMessage());

		/*
		 * s2 = connect.getHeaderField("Content-Type"); if(s2 == null ||
		 * s2.startsWith("audio") == false) return printErrMsg("Illegal Content-Type: "
		 * + s2);
		 */
		...
		// if(!acceptRanges)
		// System.out.println(url.getHost() + ": not support multi-threaded
		// downloads.");
	...
	private void waitForBuffering() throws InterruptedException {
		...
		while (bufsize < BUFFERSIZE && !eof) {
			...
			msg = String.format("\rbuffered: %6.2f%%, %6.02fKB/s ", 100f * bufsize / BUFLEN, kbps);
			if (audio != null)
