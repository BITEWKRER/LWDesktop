...
public final class Layer3 extends Layer123 {
	...
	public Layer3(Header h, IAudio audio) {
		...
		case 3:
			...
			sfbIndexLong = new int[] { 0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 116, 140, 168, 200, 238, 284, 336,
					396, 464, 522, 576 };
			sfbIndexShort = new int[] { 0, 4, 8, 12, 18, 24, 32, 42, 56, 74, 100, 132, 174, 192 };
			break;
		case 4:
			// MPEG-2, sampling_frequency=1, 24kHz
			sfbIndexLong = new int[] { 0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 114, 136, 162, 194, 232, 278, 330,
					394, 464, 540, 576 };
		...
		case 8:
			...
			sfbIndexLong = new int[] { 0, 12, 24, 36, 48, 60, 72, 88, 108, 132, 160, 192, 232, 280, 336, 400, 476, 566,
					568, 570, 572, 574, 576 };
		...
		if (isMPEG1) {
			...
			nr_of_sfb = new byte[][][] {
					...
					{ { 6, 5, 5, 5 }, { 6, 5, 7, 3 }, { 11, 10, 0, 0 }, { 7, 7, 7, 0 }, { 6, 6, 6, 3 },
							{ 8, 8, 5, 0 } },
	...
	private void requantizer(int gr, int ch, float[] xrch) {
		...
		if (ci.window_switching_flag == 1 && ci.block_type == 2) {
			...
			if (ci.mixed_block_flag == 1) {
				/*
				 * 混合块: 混合块的前8个频带是长块。 前8块各用一个增益因子逆量化，这8个增益因子 的频带总和为36， 这36条频率线用长块公式逆量化。
				 */
				rzeroBandLong = -1;
				for (; sfb < 8; sfb++) {
	...
	private void imdct36(float[] xrch, float[] preBlck, int off, int block_type) {
		...
		/*
		 * for(m = 0; m < 9; m++) { sum = 0; for(n = 0; n < 18; n += 2) sum += in[n] *
		 * cos(PI36 * (2 * m + 1) * n); out18[m] = sum; }
		 */
		...
		// >>>>>>>>> 9-point IDCT on odd
		/*
		 * for(m = 0; m < 9; m++) { sum = 0; for(n = 0;n < 18; n += 2) sum += in[n + 1]
		 * * cos(PI36 * (2 * m + 1) * n); out18[17-m] = sum; }
		 */
	...
	private void hybrid(int gr, int ch, float[] xrch, float[] preb) {
		...
		for (i = 0; i < maxi; i += 18) {
			block_type = ((ci.window_switching_flag != 0) && (ci.mixed_block_flag != 0) && (i < 36)) ? 0
					: ci.block_type;
	...
	public int decodeFrame(byte[] b, int off) {
		...
		/*
		 * part2_3: scale factors + huffman bits length: ((part2_3_bits + 7) >> 3) bytes
		 */
