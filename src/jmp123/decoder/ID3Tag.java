...
//			System.out.println(len+" -------- off = " + off);
		...
		try {
			...
			case 2583398: // TRCK: 音轨
				break;
			/*
			 * case 2614438: //USLT: 歌词 off += 4; //Languge: 4-byte len -= 4;
			 * //System.out.println(new String(b, off, len, TEXT_ENCODING[enc])); lyrics =
			 * new String(b, off, len, TEXT_ENCODING[enc]); break;
			 */
			case 2015625: // APIC
...
//								+ new String(b, off, id_part - off, TEXT_ENCODING[enc]));
				...
				if (apicData == null) {
					...
					// 内置于MP3的图片存盘
					/*
					 * try { String ext = MIMEtype.substring(MIMEtype.indexOf('/')+1);
					 * FileOutputStream fos = new FileOutputStream("apic."+ext); fos.write(b, off,
	...
	public int checkAPEtagFooter(byte[] b, int off) {
		if (b.length - off < 32)
			return 0;
		if (b[off] == 'A' && b[off + 1] == 'P' && b[off + 2] == 'E' && b[off + 3] == 'T' && b[off + 4] == 'A'
				&& b[off + 5] == 'G' && b[off + 6] == 'E' && b[off + 7] == 'X') {
